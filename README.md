...
- **Filter pills on every list page**: the Systems list has toggleable pill groups for "Uses Tool" / "Uses Use Case" / "Uses Data" plus an "Edited only" toggle; the Tools, Use Cases, and Data lists each get a search box and a "Used by System" pill filter (the Data list also gets Input/Output pills). Filters are OR-within-a-group, AND-across-groups, and a live count plus "Clear filters" button sit above the results
